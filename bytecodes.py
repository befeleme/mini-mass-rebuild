    ...
    with progressbar(py314_pkgs, item_show_func=isf) as bar:
        for pkg in bar:
            ...
            if name in processed:
                continue
            if nevr not in after(name, '2025-09-19 23:59:59'):
                ...
                if nevr not in after(name, '2025-09-19 09:39:00'):
                    torebuild.add(name)
                else:
                    inspection.add(name)
            else:
...
print(f'{len(done)} packages were build with rc3+')
print(f'{len(inspection)} packages were built on 2025-09-19 and need manual inspection')
print(f'{len(torebuild)} packages need to be rebuilt with rc3+')
